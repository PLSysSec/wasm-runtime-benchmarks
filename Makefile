...
INVOKE_wave = LD_LIBRARY_PATH=../../target/release $(SETUP_BENCH) ../../rlbox_wasm2c_sandbox/build/_deps/mod_wasm2c-src/bin/wasm2c-runner 
...
bench_raw_syscalls: build_wave
	echo "Bench: null" > raw_syscalls.txt
	cd wasi-lmbench && $(INVOKE_wave) bin/wave/lat_syscall --args="lat_syscall -N 1000000 null data/tmp.txt" --homedir="." >> ../raw_syscalls.txt
	echo "Bench: read" >> raw_syscalls.txt
	cd wasi-lmbench && $(INVOKE_wave) bin/wave/lat_syscall --args="lat_syscall -N 1000000 read data/tmp.txt" --homedir="/" >> ../raw_syscalls.txt
	echo "Bench: write" >> raw_syscalls.txt
	cd wasi-lmbench && $(INVOKE_wave) bin/wave/lat_syscall --args="lat_syscall -N 1000000 write data/tmp.txt" --homedir="/" >> ../raw_syscalls.txt
	echo "Bench: stat" >> raw_syscalls.txt
	cd wasi-lmbench && $(INVOKE_wave) bin/wave/lat_syscall --args="lat_syscall -N 1000000 stat data/tmp.txt" --homedir="." >> ../raw_syscalls.txt
	echo "Bench: fstat" >> raw_syscalls.txt
	cd wasi-lmbench && $(INVOKE_wave) bin/wave/lat_syscall --args="lat_syscall -N 1000000 fstat data/tmp.txt" --homedir="." >> ../raw_syscalls.txt
	echo "Bench: open" >> raw_syscalls.txt
	cd wasi-lmbench && $(INVOKE_wave) bin/wave/lat_syscall --args="lat_syscall -N 1000000 open data/tmp.txt" --homedir="." >> ../raw_syscalls.txt

bench_wasmtime: build_wasmtime
